...
impl AlphaStickyPathSampler {
    ...
    fn ensure_topology(&mut self, topology_index: usize, topology: &Topology) {
        if let Some(initial_topology_index) = self.topology_index {
            assert_eq!(
                topology_index, initial_topology_index,
                "alpha-sticky sampler state cannot cross topology snapshots"
            );
            return;
        }
    ...
    fn sample_new_path(&mut self, active_nodes: usize) -> Vec<usize> {
        assert!(
            (self.used_paths.len() as u128) < path_space_size(active_nodes, self.hops),
        ...
        loop {
            ...
            while path.len() < self.hops {
                let node_index = rng.gen_range(0..active_nodes);
...
impl PathSampler for AlphaStickyPathSampler {
    fn sample_path(&mut self, topology_index: usize, topology: &Topology) -> Vec<MixNode> {
        self.ensure_topology(topology_index, topology);
        ...
        let selected = if !self.leading_paths.is_empty() && rng.gen_bool(self.alpha) {
            ...
            self.sample_new_path(topology.active().len())
        };
...
fn path_space_size(active_nodes: usize, hops: usize) -> u128 {
    (0..hops).fold(1_u128, |paths, hop| {
        paths.saturating_mul((active_nodes - hop) as u128)
    })
}
