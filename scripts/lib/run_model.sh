...
case "$MODEL" in
    hidden-service) SAMPLER=fixed-path ;;
...
case "$MODEL:$SAMPLER" in
    hidden-service:fixed-path|simple:random|simple:k-hf|download-session:random|download-session:k-hf|download-session:k-w|download-session:alpha-sticky) ;;
...
mkdir -p "$RESULTS_DIR"
RUN_DIR=$(mktemp -d "$RESULTS_DIR/$(date +%Y%m%d-%H%M%S)_${MODEL}_${SAMPLER}_h${HOPS}_XXXXXX")
